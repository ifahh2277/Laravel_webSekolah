            ...
            <form action="{{ route('pembayaran.update', $pembayaran->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('siswa_id') is-invalid @enderror" 
                        ...
                        @foreach ($siswa as $item)
                            <option value="{{ $item->id }}" {{ old('siswa_id', $pembayaran->siswa_id) == $item->id ? 'selected' : '' }}>
                                {{ $item->nis }} - {{ $item->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-row">
                    <div class="form-group col-md-6">
                        ...
                        <input type="number" class="form-control @error('jumlah') is-invalid @enderror" 
                               id="jumlah" name="jumlah" step="0.01" value="{{ old('jumlah', $pembayaran->jumlah) }}" required>
                        @error('jumlah')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input type="number" class="form-control @error('tahun') is-invalid @enderror" 
                               id="tahun" name="tahun" value="{{ old('tahun', $pembayaran->tahun) }}" required>
                        @error('tahun')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('status') is-invalid @enderror" 
                        ...
                        <option value="cicilan" {{ old('status', $pembayaran->status) == 'cicilan' ? 'selected' : '' }}>Cicilan</option>
                    </select>
                    @error('status')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    @if ($pembayaran->bukti_pembayaran)
                        <div class="mb-2">
                            <img src="{{ Storage::disk('public')->url($pembayaran->bukti_pembayaran) }}" alt="Bukti Pembayaran" style="max-width: 200px;">
                        </div>
                    @endif
